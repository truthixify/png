...
impl std::convert::TryFrom<[u8; 4]> for ChunkType {
    ...
    fn try_from(bytes: [u8; 4]) -> Result<Self, Self::Error> {
        ...
        if ct.bytes[0].is_ascii_alphabetic()
            && ct.bytes[1].is_ascii_alphabetic()
            && ct.bytes[2].is_ascii_alphabetic()
            && ct.bytes[3].is_ascii_alphabetic()
        {
            return Ok(ct);
        }
...
impl std::str::FromStr for ChunkType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.len() != 4 {
            return Err("Invalid string for chunk type.".into());
        }
