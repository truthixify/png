...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        loop {
            ...
            let c = Chunk::try_from(&bytes[index..])?;
            index += c.length() as usize + 12;
            chunks.push(c);
        }
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "chunks num: {}", self.chunks.len())
    }
}
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Error> {
        ...
        if let Some(index) = self
            .chunks
            .iter()
            .position(|chunk| chunk.chunk_type() == &ct)
        {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        match ChunkType::from_str(chunk_type) {
            Ok(ct) => self.chunks.iter().find(|chunk| chunk.chunk_type() == &ct),
            Err(_) => None,
        }
    }
